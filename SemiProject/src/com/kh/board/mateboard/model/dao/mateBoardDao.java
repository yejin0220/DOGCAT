...
public class mateBoardDao {
	...
	public mateBoardDao() {
		...
		try {
			prop.loadFromXML(new FileInputStream (mateBoardDao.class.getResource("/sql/board/walk/mate-board-mapper.xml").getPath()));
	...
	public int updateAttachment(Connection conn, ArrayList<Attachment> atList) {
		...
		try {
			pstmt = conn.prepareStatement(sql);
			for(Attachment atImg : atList) {
				pstmt.setInt(1, atImg.getRefBno());
				pstmt.setString(2, atImg.getOriginName());
				pstmt.setString(3, atImg.getChangeName());
				pstmt.setString(4, atImg.getFilePath());
				pstmt.setInt(5, atImg.getFileLevel());
	...
	public int applyMate(Connection conn, int userNo, int boardNo) {
		int result = 0;
		PreparedStatement pstmt = null;
		String sql = prop.getProperty("applyMate");
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, userNo);
			pstmt.setInt(2, boardNo);
